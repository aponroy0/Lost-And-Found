...
function Maincontainer() {
      ...
      async function fetchData() {
        try {
          const url = searchTerm
            ? `https://localhost:44335/api/report/search?search=${encodeURIComponent(searchTerm)}`
  ...
  return (
    <> 
      ...
      <main className="px-6 py-8 relative z-0 pt-[80px]">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-400 text-lg mb-6">
            Connect with others to recover lost items or help return found belongings
          </p>
          
          {/* Search box centered in hero section */}
          <div className="flex justify-center mb-6">
            <div className="w-full max-w-md">
              <div className="rounded-lg px-4 py-2 flex items-center gap-3" style={{ backgroundColor: 'oklch(37.3% 0.034 259.733)' }}>
          ...
          >
            <div className="p-4">
              <div className="space-y-2">
                {loading ? (
                  <p className="text-center text-white mt-10">Loading Items...</p>
                ) : (
                  items.map((item) => (
                    <ItemCard
                      key={item.ItemId}
      ...
      {selectedItem && (
        <>
          {Token && <ItemModal item={selectedItem} onClose={() => setSelectedItem(null)} />}
